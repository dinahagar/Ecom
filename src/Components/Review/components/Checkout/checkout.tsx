...
const Checkout = () => {
  ...
  const Quantity = useSelector((state: RootState) => state.camera.quantities);
  ...
  const cameraPrices = calculatePrices(cameraData?.items, Quantity);
  const sensorPrices = calculatePrices(sensorsData?.items, Quantity);
  const protectionPrices = calculatePrices(protectionData?.items, Quantity);

  const planPrices = {
    old: selectedType?.length > 0
      ? Number(selectedType?.[0]?.oldPrice?.replace("$", "").replace("/mo", ""))
      : 0,
    new: selectedType?.length > 0
      ? Number(selectedType?.[0]?.newPrice?.replace("$", "").replace("/mo", ""))
      : 0,
  };
  ...
  console.log('cameraPrices', cameraPrices);
  console.log('sensorPrices', sensorPrices);
  console.log('protectionPrices', protectionPrices);
  console.log('planPrices', planPrices);
  console.log('CheckoutOldPrice', CheckoutOldPrice);
  console.log('CheckoutNewPrice', CheckoutNewPrice);
  
console.log(selectedType?.lenght > 0 ? selectedType[0] : 0);
      ...
      <CheckoutSection>
        <CheckImgDiv>
          ...
          <div style={{ textAlign: "end" }}>
            ...
            <CheckoutPrice>
              <OldCheckout>
                {CheckoutOldPrice > 0 ? `$${CheckoutOldPrice.toFixed(2)}` : null}
              </OldCheckout>
              <NewCheckout>
                {CheckoutNewPrice > 0 ? `$${CheckoutNewPrice.toFixed(2)}` : null}
              </NewCheckout>
            </CheckoutPrice>
          </div>
        </CheckImgDiv>
        ...
        <div>
          <SaveButton>Save my system for later</SaveButton>
        </div>
      </CheckoutSection>
    </>
  );
